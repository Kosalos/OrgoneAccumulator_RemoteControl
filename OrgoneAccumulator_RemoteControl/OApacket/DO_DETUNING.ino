...
//Serial.println(POT_WAVE_HI>>8);
  switch (FX) {
    case 0: //symetrical detune - primes (bipolar)
    mixEffectUp = mixEffect * 0.77; 
    mixEffectDn =  (2047 - mixEffect) * 0.97; 
    
      bipolarFX = (constrain((((4095 - aInEffectReading )<< 1) + (POT_EFFECT - 4095)), -4095, 4095));
    ...
    mixEffectDn =  (2047 - mixEffect) * 0.97; 
      ...
      o1.phaseOffset = (uint32_t)( constrain(((4095 - aInEffectReading) + (POT_EFFECT)), 0, 8190))<<20;//difference between the waves.
...
mixEffectUp = mixEffect * 0.97; 
 ...
 mixEffectUp = mixEffect * 0.97; 
    ...
    mixEffectDn =  (2047 - mixEffect) * 0.97; 
      ...
      chordArrayOffset = ((constrain(((4095 - aInEffectReading) + (POT_EFFECT)), 0, 8190)) >> 10) * 3;
      for (int i = 0; i <= 3; i++) {
    ...
    mixEffectDn =  (2047 - mixEffect) * 0.97; 
      ...
      Lbuh = (POT_WAVE_LO >> 9) * 9;
      Mbuh =  (POT_WAVE_MID >> 9) * 9;
      Hbuh =  (POT_WAVE_HI >> 9) * 9;
      break;
    ...
    case 6: //delay (bipolar)
     ...
     mixEffectDn =  (2047 - mixEffect) * 0.97;
      ...
      GRADUALWAVE();
      o3.phaseOffset = (uint32_t)(POT_WAVE_LO) << 20;
      delayTime = constrain(((averageaInRAv - 4065) + averageratio), 8, 8190); //For Feedback altFX ratio knob
      delayFeedback = (int32_t)((POT_EFFECT>>1)-2048)+(int32_t)((4095 - aInEffectReading)>>1); //detune become feedback
    ...
    GRADUALWAVE_D();
      ...
      floats[0] = (float)((constrain(((4095-aInModIndex ) + (POT_INDEX)), 0.0, 8191.0))/8192.0); //make a log pot of index.
      floats[0] =  (floats[0]*floats[0]*floats[0])*524288.0;
      o6.phase_increment = ((int32_t)floats[0])>>5; //the amount of xmod
      
      drum_a = EffectEnOn_B * (POT_WAVE_MID << 10); //drum hold time

      o1.amp = (constrain(((AInRawFilter-4095) + POT_FREQ), 0, 8191))>>3;//amount of pbend on fm freq pot

      drum_d = map(( POT_WAVE_MID), 1, 8191, 24000, 24); //decaY 1
      if (EffectEnOn_A) drum_d = drum_d + (o1.phase_increment>>12);
      drum_d = signed_multiply_32x16t((drum_d * drum_d), drum_d << 13) + (32 << 10);
      
      drum_d2 = (map( POT_MOD, 1, 8191, 32000, 24)); //decay 2
      drum_d2 = signed_multiply_32x16t((drum_d2 * drum_d2), drum_d2 << 12) + (32 << 10);
      
      bipolarFX = (constrain((((4095 - aInEffectReading) << 1) + (POT_EFFECT - 4095)), -4095, 4095));//detune amounts
