...
void ASSIGNINCREMENTS_P() { //--------------------------------------------for pulsar
  ...
  if (FMFixedOn) {
    ...
    if (FX == 6) FMMult = (float)((((POT_MOD) * 1.5 )) + 1.0);
    ...
    if (FX == 6) FMMult = (float)((((POT_MOD >> 4) / 14.0)) + 1.0) ;    
...
void ASSIGNINCREMENTS_D() { //--------------------------------------------------------delay
  ...
  switch (oscMode) {
    case 0:
      ...
      if (FMFixedOn) {
        avgcubing = (float)(POT_MOD / 500.0); //change to adjust "LFO" in fm fixed
...
FMX_HiOffset = (float)(1.0 + ((float)(mixHi) * FMX_HiOffsetCont)/2048.0);
      ...
      if (FMFixedOn) {
        float avgcubing = POT_MOD / 500.0; //change to adjust "LFO" in fm fixed
    ...
    case 1://cz
      ...
      if (FMFixedOn) {
        FMMult = (float)((((POT_MOD >> 3) / 1.1)) + 1.0); //CZ + fixed + free
        osc_mult[0] = 4;
        osc_mult[1] = FMMult;
        o1.phase_increment = inputConverter * osc_mult[0];
        o2.phase_increment = inputConverterF * osc_mult[1];
        ...
        FMMult = (float)((((POT_MOD >> 5))) + 1.0); //CZ + free + free
        osc_mult[0] = 4;
        osc_mult[1] = FMMult;
        o1.phase_increment = inputConverter * osc_mult[0];
        o2.phase_increment = inputConverter * osc_mult[1];
...
FMX_HiOffset = (float)(1.0 + ((float)(mixHi) * FMX_HiOffsetCont)/2048.0);
      ...
      if (FMFixedOn) {
        FMMult = (float)((((POT_MOD >> 3) / 1.1)) + 1.0); 
        osc_mult[0] = 4;
        osc_mult[1] = FMMult;
