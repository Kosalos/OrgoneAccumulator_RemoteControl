void loop() {
  ...
  // DRUMBEAT $$$$$$$$$$$$$$$$$$$$
  // inCVraw is hardware reading plus offset received from drumBeat app
  v = (int)analogRead(A0);              // raw InCV
  vOffset = v + (drumBeat.slider[0] - 4096)/ 4; // CV offset from DrumBeat app
  if (vOffset < 0) vOffset = 0; else if (vOffset > 8191) vOffset = 8191;

  inCVraw = (float)vOffset;     // main v/oct CV in. only use 13 bits of analog in SEPERATE AINS WITH CODE

    // inputCV voltage sent to DrumBeat app is  inputCV + tune + fineTune pots
  cv = (uint16_t)(v + (POT_TUNE >> 1) + (POT_TUNE_FINE >> 4));

  if (abs(oldInCV - cv) > 25) {       // 25 = allow for noise jitter in A/D reading
    oldInCV = cv;
    sendInCV(cv);
  }
  // DRUMBEAT $$$$$$$$$$$$$$$$$$$$
  ...
  dtuner = (float)((((int)(POT_TUNE / 227)) * (conf_NoteSize * tuneStep)) + ((int)(POT_TUNE_FINE / 28))); //coarse and fine tuning
  tuner = (float)(inCV + dtuner);
  //comment out above line and uncomment following line for analog style non stepped tuning
  //tuner = inCV+(POT_TUNE>>1)+(POT_TUNE_FINE>>4);
